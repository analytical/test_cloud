---
title: ¿Cómo "demuestro" que mi curva de calibracion es lineal?
author: cgomez
date: '2017-08-23'
...
tags:
  - calibracion
  - linealidad
summary: 'En este post intentaremos derribar el mito del coeficiente de 
correlación con _muchos_ 9s como prueba de linealidad. Además, presentaremos 
dos test formales para evaluar el modelo de calibración lineal en química 
analítica.'
---

<!-- Promedín: Lo siento, no se puede demostrar eso. Fin del Post.\ -->
...
¿No me cree? Le dejo este [link](https://1drv.ms/x/s!AuF6FPVWruwDtfpb0dC8SSaBCChfzA) para que descargue los datos en archivo Excel
...
ambos tests estadísticos intentan intentan dirimir entre dos hipótesis:
...
También debemo hacer la siguiente acotación: En **estadística** 
un modelo lineal es
aquel en sus parámetros son lineales. Por ejemplo, la curva de calibración lineal
...
library(alr3) # Cargamos el package 'alr3'
pureErrorAnova(fit.cal)
...
library(alr3)
lof.test <- pureErrorAnova(fit.cal)
kable(lof.test,
      digits = c(0, 0, 0, 0, 2),
...
bajo la columna _Pr(>F)_ = `r round(tidy(lof.test)[3, 6], 2)`. La interpretación
...
calibrantes. Si observa la ecuación \ el concepto de residuo es el mismo para
...
(ICP-MS) exista una relación "perfectamente" lineal entre absorbacia (cuentas) 
y concentración. Lo que hizo Lambert-Beer (o cualquier científico que 
